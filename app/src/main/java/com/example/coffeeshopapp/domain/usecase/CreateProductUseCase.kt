...
class CreateProductUseCase(private val repository: ProductRepository) {
    suspend operator fun invoke(requestDto: ProductRequestDto, image: MultipartBody.Part?): ApiResponseDto<ProductDto> {
        val jsonRequest = Gson().toJson(requestDto)
        val requestBody = jsonRequest.toRequestBody("application/json".toMediaTypeOrNull())
        return repository.createProduct(requestBody, image)
    }
}
